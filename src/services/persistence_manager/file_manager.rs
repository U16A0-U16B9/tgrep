...
const DATA_CHATS: &str = "chats.json";
...
impl PersistenceManager for FileManager {
    fn load_data(data_type: DataType) -> Option<String> {
        ...
        match file_text {
            Ok(_file_text) => {
                info!("data {} loaded", data_type.to_string());
                Some(_file_text)
            }
            Err(err) => {
                error!("cannot load data {}", data_type.to_string());
                error!("{}", err.to_string());
                None
            }
        }
    ...
    fn save_data(data_type: DataType, data: String) {
        ...
        match result {
            Ok(ok) => {
                info!("data {} saved", data_type.to_string());
                ok
            }
            Err(err) => {
                error!("cannot save data {}", data_type.to_string());
                error!("{}", err.to_string());
            }
        }
    }
    ...
    fn load_config(config_type: ConfigType) -> Option<String> {
        ...
        match file_text {
            Ok(_file_text) => {
                info!("config {} loaded", config_type.to_string());
                Some(_file_text)
            }
            Err(err) => {
                error!("cannot load {}", config_type.to_string());
                error!("{}", err.to_string());
                None
            }
        }
    ...
    fn save_config(config_type: ConfigType, config: String) {
        ...
        match result {
            Ok(ok) => {
                info!("config {} saved", config_type.to_string());
                ok
            }
            Err(err) => {
                error!("cannot save config {}", config_type.to_string());
                error!("{}", err.to_string())
            }
        }
    }
...
fn get_data_filename(data_type: &DataType) -> PathBuf {
    match data_type {
        ...
        DataType::Chats => get_data_dir().as_path().join(DATA_CHATS),
    }
}
...
fn get_config_dir() -> PathBuf {
    ...
    match project_dir {
        None => get_current_dir(),
        Some(project_dir) => project_dir.config_dir().to_path_buf(),
    }
}
